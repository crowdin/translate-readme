...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const core = __importStar(__nccwpck_require__(2186));
const translator_1 = __nccwpck_require__(3296);
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const translator = new translator_1.Translator(credentialsConfig, config, new logger_1.Logger());
...
const zip_1 = __nccwpck_require__(236);
...
class Translator {
    constructor(credentials, config, logger) {
        ...
        this.config = config;
        this.logger = logger;
        this.crowdin = new crowdin_api_client_1.default({
            token: this.credentials.token,
            organization: this.credentials.organization
        });
        this.writer = new writer_1.Writer(credentials, config, logger);
    }
    translate() {
        return __awaiter(this, void 0, void 0, function* () {
            this.logger.log('info', 'Start...');
            const project = yield this.crowdin.projectsGroupsApi.getProject(this.credentials.projectId);
            this.writer.setProjectLanguages(project.data.targetLanguages);
            if (this.config.uploadSources) {
                yield this.writer.addLanguageSwitcher(this.config.file);
                yield this.uploadSources();
            }
            if (this.config.downloadTranslations) {
                const translationsUrl = yield this.downloadTranslations();
                const translationFiles = yield (0, zip_1.downloadZipAndUnzip)(translationsUrl, this.config.destination);
                translationFiles.map((file) => __awaiter(this, void 0, void 0, function* () {
                    yield this.writer.addLanguageSwitcher(`${this.config.destination}/${file}`);
                }));
            }
    ...
    uploadSources() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const readmeFile = yield this.getFile();
            const exportPattern = '/README.%locale%.md';
            const storageFile = yield this.crowdin.uploadStorageApi.addStorage((0, path_1.baseName)(this.config.file), fs.readFileSync(this.config.file));
            let branchId = undefined;
            if (this.config.branch) {
                let branch = yield this.getBranch();
                if (!branch) {
                    branch = (yield this.crowdin.sourceFilesApi.createBranch(this.credentials.projectId, {
            ...
            if (readmeFile) {
                yield this.crowdin.sourceFilesApi.updateOrRestoreFile(this.credentials.projectId, readmeFile.id, {
                ...
                yield this.crowdin.sourceFilesApi.createFile(this.credentials.projectId, {
                    storageId: storageFile.data.id,
                    name: (0, path_1.baseName)(this.config.file),
                    branchId: branchId,
                    exportOptions: {
                        exportPattern: exportPattern
                    }
    ...
    downloadTranslations() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const result = yield this.crowdin.translationsApi.buildProject(this.credentials.projectId, request);
            let status = result.data.status;
            while (status === 'inProgress') {
                const progress = yield this.crowdin.translationsApi.checkBuildStatus(this.credentials.projectId, result.data.id);
                status = progress.data.status;
                this.logger.log('info', `Progress: ${progress.data.progress}%`);
            }
            const translations = yield this.crowdin.translationsApi.downloadTranslations(this.credentials.projectId, result.data.id);
    ...
    getBranch() {
        return __awaiter(this, void 0, void 0, function* () {
            const branches = yield this.crowdin.sourceFilesApi.listProjectBranches(this.credentials.projectId);
            const crowdinBranch = branches.data.find(branch => branch.data.name === this.config.branch);
            return crowdinBranch === null || crowdinBranch === void 0 ? void 0 : crowdinBranch.data;
        });
    }
    getCrowdinFilePath() {
        const fileName = (0, path_1.baseName)(this.config.file);
        if (this.config.branch) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.toPlatformPath = exports.baseName = exports.generateRelativeFileLink = void 0;
const path = __importStar(__nccwpck_require__(1017));
const generateRelativeFileLink = (currentFilePath, destinationFilePath) => {
    return path.relative(path.dirname(currentFilePath), destinationFilePath);
};
exports.generateRelativeFileLink = generateRelativeFileLink;
const baseName = (filePath) => {
    return filePath.split('/').pop() || filePath;
};
exports.baseName = baseName;
const toPlatformPath = (pth) => {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.downloadZipAndUnzip = void 0;
const fs = __importStar(__nccwpck_require__(7147));
const adm_zip_1 = __importDefault(__nccwpck_require__(6761));
const axios_1 = __importDefault(__nccwpck_require__(8757));
const path_1 = __nccwpck_require__(8040);
const downloadZipAndUnzip = (zipUrl, destination) => __awaiter(void 0, void 0, void 0, function* () {
    const response = yield axios_1.default.get(zipUrl, { responseType: 'arraybuffer' });
    const zipFilePath = (0, path_1.toPlatformPath)(`Translations-${Date.now()}.zip`);
    fs.writeFileSync(zipFilePath, response.data);
    const zip = new adm_zip_1.default(zipFilePath);
    zip.extractAllTo((0, path_1.toPlatformPath)(destination), true);
    const entries = zip.getEntries().map(entry => entry.entryName);
    fs.unlinkSync(zipFilePath);
    return entries;
});
exports.downloadZipAndUnzip = downloadZipAndUnzip;
...
class Writer {
    ...
    addLanguageSwitcher(file) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.config.languageSwitcher) {
                return;
            }
            this.logger.log('info', `Adding language switcher to ${file}...`);
            let fileContents = fs_1.default.readFileSync((0, path_1.toPlatformPath)(file)).toString();
            if (!fileContents.includes(this.placeholderStart) || !fileContents.includes(this.placeholderEnd)) {
                this.logger.log('warning', `Skipped! Please add ${this.placeholderStart} and ${this.placeholderEnd} to your README.md`);
                return;
            }
            ...
            const switcher = yield this.renderSwitcher(file);
            fileContents = `${fileContents.slice(0, sliceFrom)}\n${switcher}\n${fileContents.slice(sliceTo)}`;
            this.logger.log('debug', fileContents);
            fs_1.default.writeFileSync((0, path_1.toPlatformPath)(file), fileContents);
        });
    }
    renderSwitcher(file) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.projectLanguages.map(language => {
                ...
                if ((0, path_1.baseName)(link) === (0, path_1.baseName)(file)) {
                    languages.push(`**${language.name}**`);
                }
                else {
                    languages.push(`[${language.name}](${(0, path_1.generateRelativeFileLink)(file, link)})`);
                }
            });
...
class Applications extends core_1.CrowdinApi {
     ...
     */
    installApplication(applicationUrl, permissions) {
        const url = `${this.url}/applications/installations`;
...
class TranslationMemory extends core_1.CrowdinApi {
    ...
    listTmSegments(tmId, options) {
        ...
        url = this.addQueryParam(url, 'croql', options === null || options === void 0 ? void 0 : options.croql);
        return this.getList(url, options === null || options === void 0 ? void 0 : options.limit, options === null || options === void 0 ? void 0 : options.offset);
    }
    /**
     * @param tmId tm identifier
